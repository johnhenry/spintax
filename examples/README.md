...
Runnable, self-verifying examples. Each one asserts the behavior it
demonstrates and exits non-zero on failure. Run any example directly
(`node examples/01-generate-every-variation.mjs`) or run them all with
`npm run examples` from the repo root.
...
| [demo.mjs](./demo.mjs) | Kitchen-sink tour of the whole API (`npm run demo`). **Not** part of `npm run examples`. |
